	...
	$( "input" ).on( "click", function() {
		...
		if(anho.val() != ''){
			...
			if (opciones == "p1") {
				...
				$.post("cmosproyect/p1",postForm,function(res){
						...
						if (res.length>3) {
							...
							for (var i = 0; i <  datos.length ; i++) {
								...
								if(aux != datos[i]['descripcion']){
									informacion+= '<tr>';
									informacion+= '<td colspan="2" class="red-header">Total '+datos[i-1]['numcategoria']+" "+datos[i-1]['descripcion']+'</td>';
									...
									informacion+= '<td class="red-header">'+parseFloat(d).toFixed(2)+'</td>';
									informacion+= '</tr>';
									}// fin if antes del ultimo arreglo

								}// fin else opcion para mostrar normal y no total
						...
						if (res.length>3) {
		 				...
		 				if(opciones == "p3"){
		 					var v2 = 2;
		 					var postForm ={'v3' : v3};
		 					$.post("cmosproyect/p3",postForm,function(res){
								...
								var corrida = i;
									...
									for (var j = 0; j < datos.length ; j++) {
										...
										if(corrida != j){
											...
											if(datos[i]['idproyecto'] == datos[j]['idproyecto']){
				...
				$.post("cmosproyect/r1",postForm,function(res){
				...
				$.post("cmosproyect/r1",postForm,function(res){
						...
						if (res.length>3) {
							...
							$('#mejorv3 tbody').append(informacion);	
		 					...
		 					var postForm ={'v3' : v3};
		 					$.post("cmosproyect/r3",postForm,function(res){
				...
				$.post("cmosproyect/mv1",postForm,function(res){
				...
				$.post("cmosproyect/mv1",postForm,function(res){
						...
						if (res.length>3) {
							...
							$('#mejorv3 tbody').append(informacion);	
		 					...
		 					var postForm ={'v3' : v3};
		 					$.post("cmosproyect/mv3",postForm,function(res){
